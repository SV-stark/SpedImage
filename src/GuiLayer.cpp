...
void GuiLayer::Render(App *app) {
  ...
  if (ShowSidebar) {
    ...
    ImGui::SetNextWindowSize(ImVec2(SidebarWidth, ImGui::GetIO().DisplaySize.y - menuBarHeight));
    ...
    std::string currentName = std::filesystem::path(currentPath).filename().string();
    ...
    for (const auto &file : files) {
      ...
      bool hovered = ImGui::IsMouseHoveringRect(cursorPos, ImVec2(cursorPos.x + itemSize.x, cursorPos.y + itemSize.y));
      
      if (isSelected) {
        drawList->AddRectFilled(cursorPos, ImVec2(cursorPos.x + itemSize.x, cursorPos.y + itemSize.y), 
                               ImGui::ColorConvertFloat4ToU32(ImVec4(0.29f, 0.56f, 0.85f, 0.25f)), 6.0f);
        drawList->AddRect(cursorPos, ImVec2(cursorPos.x + itemSize.x, cursorPos.y + itemSize.y), 
                         ImGui::ColorConvertFloat4ToU32(Colors::AccentBlue), 6.0f, 0, 1.5f);
      } else if (hovered) {
        drawList->AddRectFilled(cursorPos, ImVec2(cursorPos.x + itemSize.x, cursorPos.y + itemSize.y), 
                               ImGui::ColorConvertFloat4ToU32(ImVec4(1.0f, 1.0f, 1.0f, 0.05f)), 6.0f);
      }
      
      // Invisible button for click detection
      ImGui::InvisibleButton("file", itemSize);
      if (ImGui::IsItemClicked()) {
        std::string parent = std::filesystem::path(currentPath).parent_path().string();
      ...
      ImU32 iconColor = ImGui::ColorConvertFloat4ToU32(isSelected ? Colors::AccentBlue : Colors::TextMuted);
      drawList->AddRectFilled(iconPos, ImVec2(iconPos.x + iconSize.x, iconPos.y + iconSize.y), 
                             iconColor, 4.0f);
      
      // File name
      ImVec2 textPos = ImVec2(cursorPos.x + 40.0f, cursorPos.y + (itemSize.y - ImGui::GetFontSize()) * 0.5f);
      ImU32 textColor = ImGui::ColorConvertFloat4ToU32(isSelected ? Colors::TextPrimary : Colors::TextSecondary);
      ...
      if (textWidth > maxTextWidth) {
        while (displayName.length() > 3 && ImGui::CalcTextSize((displayName + "...").c_str()).x > maxTextWidth) {
...
void GuiLayer::RenderStatusBar(App* app, std::shared_ptr<class Image> img, float sidebarWidth, float menuHeight, float viewportW, float viewportH) {
  if (!img) return;
  ...
  drawList->AddRect(pos, ImVec2(pos.x + size.x, pos.y + size.y), 
                   ImGui::ColorConvertFloat4ToU32(Colors::Border), 8.0f, 0, 1.0f);
  
  // Info text
  std::string info = std::to_string(img->GetWidth()) + "x" + std::to_string(img->GetHeight()) + " px";
  ImVec2 textPos = ImVec2(pos.x + 16.0f, pos.y + (size.y - ImGui::GetFontSize()) * 0.5f);
  drawList->AddText(textPos, ImGui::ColorConvertFloat4ToU32(Colors::TextSecondary), info.c_str());
}
