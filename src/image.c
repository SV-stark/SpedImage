...
ImageFormat image_detect_format(const char* filename) {
    ...
    if (strcasecmp(ext, "jpg") == 0 || strcasecmp(ext, "jpeg") == 0) return IMG_FORMAT_JPEG;
    if (strcasecmp(ext, "png") == 0) return IMG_FORMAT_PNG;
    if (strcasecmp(ext, "bmp") == 0) return IMG_FORMAT_BMP;
    if (strcasecmp(ext, "gif") == 0) return IMG_FORMAT_GIF;
    if (strcasecmp(ext, "tga") == 0) return IMG_FORMAT_TGA;
    if (strcasecmp(ext, "hdr") == 0) return IMG_FORMAT_HDR;
    if (strcasecmp(ext, "psd") == 0) return IMG_FORMAT_PSD;
    if (strcasecmp(ext, "pic") == 0) return IMG_FORMAT_PIC;
    if (strcasecmp(ext, "ppm") == 0 || strcasecmp(ext, "pgm") == 0 || strcasecmp(ext, "pbm") == 0) return IMG_FORMAT_PNM;
    if (strcasecmp(ext, "svg") == 0) return IMG_FORMAT_SVG;
    if (strcasecmp(ext, "heic") == 0 || strcasecmp(ext, "heif") == 0) return IMG_FORMAT_HEIC;
    if (strcasecmp(ext, "avif") == 0) return IMG_FORMAT_AVIF;
    if (strcasecmp(ext, "webp") == 0) return IMG_FORMAT_WEBP;
    if (strcasecmp(ext, "tiff") == 0 || strcasecmp(ext, "tif") == 0) return IMG_FORMAT_TIFF;
    if (strcasecmp(ext, "raw") == 0 || strcasecmp(ext, "cr2") == 0 || strcasecmp(ext, "nef") == 0 ||
        strcasecmp(ext, "arw") == 0 || strcasecmp(ext, "dng") == 0 || strcasecmp(ext, "orf") == 0 ||
        strcasecmp(ext, "raf") == 0 || strcasecmp(ext, "pef") == 0 || strcasecmp(ext, "x3f") == 0) {
...
bool image_save(Image* img, const char* filename) {
    if (!img || !img->raw_data) return false;
    
    const char* ext = get_extension(filename);
    
    if (strcasecmp(ext, "png") == 0) {
        return stbi_write_png(filename, img->width, img->height, 4, img->raw_data, img->width * 4) != 0;
    } else if (strcasecmp(ext, "jpg") == 0 || strcasecmp(ext, "jpeg") == 0) {
        return stbi_write_jpg(filename, img->width, img->height, 4, img->raw_data, 90) != 0;
    } else if (strcasecmp(ext, "bmp") == 0) {
        return stbi_write_bmp(filename, img->width, img->height, 4, img->raw_data) != 0;
    } else if (strcasecmp(ext, "tga") == 0) {
        return stbi_write_tga(filename, img->width, img->height, 4, img->raw_data) != 0;
    }
...
Image* cache_get(ImageCache* cache, int index) {
    if (index < 0 || index >= cache->count) return NULL;
...
bool cache_add(ImageCache* cache, Image* img) {
    ...
    while (cache->current_memory > cache->max_memory && cache->count > 1) {
        if (cache->count > 0 && cache->images[0]) {
            ...
            memmove(&cache->images[0], &cache->images[1], (cache->count - 1) * sizeof(Image*));
            cache->count--;
            if (cache->current_index > 0) cache->current_index--;
        }
    }
